...
//PHP script που φτιάχνει to geoJSON για τις πληροφορίες
//του χάρτη
...
try {
  ...
  //Queries για την ανάκτηση των πληροφοριών από την βάση δεδομένων
  ...
  $truck_info = $db->prepare("SELECT * FROM vehicle where vehicle_id=?");
  $truck_info->bind_param("i", $_SESSION["truck_id"]);
  $truck_info->execute();
  $truck_response = $truck_info->get_result();

  $truck_check = 0;

  if ($truck_response->num_rows > 0) {
    $truck_row = $truck_response->fetch_assoc();
    $truck_check = 0;
    $truck_array = array(
